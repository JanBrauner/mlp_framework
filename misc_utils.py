...
def create_central_region_slice(image_size, size_central_region):
    """
    create slice of the central region of an image (dimensions (HxW)), when the size of the central region is central_region_size (HxW)
    ...
    central_region_slice = np.s_[:, 
                                 math.ceil(margins[0]):math.ceil(image_size[0]-margins[0]), 
